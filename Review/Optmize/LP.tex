...
$x_{n+i}=b_i-f_i(x)$。记目标函数值为$z$，则也可以引入等式$z=c^Tx$。实际上基本变量
与目标函数值都被表示为常数+非基本变量线性加权和的形式。在此称等式左边的变量为
{\bfseries 基本变量}，等式右边变量称为{\bfseries 非基本变量}。等式组的变量不同时
出现于等号两边，基本变量也不会出现两次。事实上，这些约束把基本变量与目标函数值表示为
非基本变量的线性加权和。{\bfseries 注意等式左边的变量不等同于松弛变量，因为单纯形算法
的转动操作会使变量的位置改变。}
\subsection{单纯形算法}
...
\begin{displaymath}
    A[i][0]+\textrm{目标函数值}z=\sum_{j=1}^n{A[i][j]x_{id[j]}}
\end{displaymath}
该式可由$z=c^Tx$推导。
...
\begin{itemize}
    ...
    \item 其余行：由于$A[l][0]/A[l][e]$最小，即转动替出变量所在行后
...
证明：由于所有系数非正，目标函数值只取决于非基本变量的值，且非基本变量
均为0，所以只能提升任意非基本变量的值。即使提升非基本变量的值仍然得到可行解，
目标函数值也是不减的。
\paragraph{选取替入/替出变量}
...
找到一个初始基本可行解意味着要让松弛形中约束的常数项非负。
\paragraph{辅助线性规划}
首先检查初始松弛形是否已经对应了基本可行解，记$k$满足$A[k][0]$最小。

然后引入一个新变量$x_0$，和原有的线性规划组成一个新线性规划：
\begin{itemize}
    \item 最大化$-x_0$
...
证明：
\begin{itemize}
    \item 替出变量所在行：$L[0]$为负且$L[e]=-1$，转动后$L[0]'$变正。
...
若线性规划的系数矩阵$A$为全幺模矩阵，可以把目标函数中系数为0的变量去除。
