...
void access(int u) {
    ...
    do {
        ...
        if(rs) {
            T[u].icnt += T[rs].rsiz;
            T[u].isiz.push(T[rs].rsiz);
        }
        rs = v;
        if(rs) {
            T[u].icnt -= T[rs].rsiz;
            T[u].isiz.pop(T[rs].rsiz);
        }
        update(u);
        v = u;
        u = T[u].p;
...
void link(int u, int v) {
    ...
    T[u].p = v;
    T[v].icnt += T[u].rsiz;
    T[v].isiz.push(T[u].rsiz);
    update(v);
}
int query(int u) {
    access(u);
    splay(u);
    return std::max(T[u].isiz.top(), T[ls].rsiz);
}
struct G {
    struct Edge {
        int to, nxt;
...
int crt, nrt, dst;
void clear(int u) {
    T[u].p = ls = rs = T[u].icnt = T[u].rev = 0;
    T[u].rsiz = 1;
    T[u].isiz.clear();
}
