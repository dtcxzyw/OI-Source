...
Int64 S(Int64 i, int j) {
    if(i <= 1 || p[j] > n)
        return 0;
    Int64 res = getG(i) - fac * (j - 1);
    for(int k = j; k <= pcnt; ++k) {
        Int64 cp = p[k], p1 = cp, p2 = p1 * p1;
        if(p2 > i)
            break;
        for(Int64 e = fac; p2 <= i;
            e += K, p1 = p2, p2 *= cp) {
            res += e * S(i / p1, k + 1) + (e + K);
        }
    }
    return res;
}
Int64 q[sqsiz * 2];
Int64 solve(Int64 x, Int64 y) {
    n = x, sqn = sqrt(x), K = y, fac = y + 1;
    Int64 dv = 1;
    int m = 0;
    while(dv <= n) {
        Int64 val = n / dv;
